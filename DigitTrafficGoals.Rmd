---
title: "Commercial Goals: Digital Traffic"
...
load(file = here::here("data", "CleanDigitalTraffc.rds"))

# ---- DifferentFC Combined -------

CombFC <-
  bind_rows(
    forecast1 %>%  filter(lubridate::year(as.Date(ds)) == 2022) %>% select(ds, yhat) %>% mutate(Model = "2022FC_Mid"),
    forecast2 %>%  filter(lubridate::year(as.Date(ds)) == 2022) %>% select(ds, yhat) %>% mutate(Model = "2022FC_Flat"),
...
Act_FC <- 
  ...
  summarise(TotalTraff = sum(Sessions)) %>% 
  ungroup() %>% 
...
write_csv(Act_FC, here::here("output", "DigitTraffic_Goals.csv"))
...
(final_table <-
  ...
  tab_style(
    ...
    locations = list(cells_body(columns = c(`2022FC_Flat`)))) %>% 
   tab_style(
  ...
  tab_style(
    ...
    locations = cells_body(columns = c(`2022FC_High`, `2022FC_Flat`, `2022FC_Mid`))) %>% 
...
* __Model 1 (Low/Flat):__ Covid data included, and covid period counted as 3/1/2020 - 5/1/2021, flattened trend.    
    * This model produces the smallest forecasted traffic.
* __Model 2 (Mid-point):__ Covid data included, includes growth trend  
    * This model producess a forecasted traffic that is close to 2019 traffic.
...
### Data Generation {-} 
Web traffic data, as measured be Google web sessions.

### Data Pull from DB {-}
For forecasting purposes we have pulled data for all centers from 2009 until now (2022.01.26).

__Source:__ Laura Simmons (server: ___)  

__Tables:__ N/A

Not sure if there was any additional filtering logic done on either LS side or in the data that is available in Google Analytics.
...
Hist <- CleanTraffc %>% 
  ...
  group_by(month, All) %>% 
  summarise(traff = sum(Sessions,na.rm=T)) %>% 
  ungroup()
...
## Export: Digital Traffic {-}

Daily raw counts for Tangeroutlet.com since 4/2/2009. 
