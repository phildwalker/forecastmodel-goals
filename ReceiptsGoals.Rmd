---
title: "Commercial Goals: Receipts Program"
...
load(file = here::here("data", "CleanRecpts.rds"))
...
Act_FC <- 
  ...
  summarise(TotalTraff = sum(UniqCust)) %>% 
  ungroup() %>% 
...
write_csv(Act_FC, here::here("output", "Receipts_Goals.csv"))
...
### Data Pull from DB {-}
For forecasting purposes we have pulled data for all centers until now (2022.01.28). The forecast model excluded any data before 2015 as values were much lower.
...
Hist <- CleanRecpts %>% 
  ...
  group_by(month, All) %>% 
  summarise(traff = sum(UniqCust,na.rm=T)) %>% 
  ungroup()
...
Traffc <-
  ...
  mutate(ds = Date) %>% 
  group_by(ds) %>% 
  summarise(y = sum(UniqCust, na.rm=T)) %>% 
  ungroup()
...
ggplot()+
  ...
  labs(title = "Forecasted Portfolio Vehicle Traffic: Model 1 (Mid: Covid + Keeps Trend)", 
       subtitle = "Started Forecast Model in 2015",
