---
title: "Commercial Goals: Retailer Revenue (Sales)"
...
load(file = here::here("data", "CleanSales.rds"))

# ---- DifferentFC Combined -------

CombFC <-
  bind_rows(
    forecast1 %>%  filter(lubridate::year(as.Date(ds)) == 2022) %>% select(ds, yhat) %>% mutate(Model = "2022FC_High"),
    forecast2 %>%  filter(lubridate::year(as.Date(ds)) == 2022) %>% select(ds, yhat) %>% mutate(Model = "2022FC_Low"),
    forecast3 %>%  filter(lubridate::year(as.Date(ds)) == 2022) %>% select(ds, yhat) %>% mutate(Model = "2022FC_Mid")
...
Act_FC <- 
  ...
  summarise(TotalTraff = sum(sales)) %>% 
  ungroup() %>% 
...
write_csv(Act_FC, here::here("output", "Revenue_Goals.csv"))


(final_table <-
  Act_FC %>% 
  mutate_if(is.integer, as.numeric) %>% 
  # mutate_if(is.numeric, ~./1000000000) %>%
  gt(rowname_col = "MonthName") %>% 
  fmt_currency(columns = 2:9,  decimals = 1,  suffixing = TRUE,currency = "USD") %>% #pattern = "{x}B"
  tab_style(
  ...
  summary_rows(
    # groups = TRUE,
    columns = c(2:9), fns = list(total = "sum"),  formatter = fmt_currency, currency = "USD", #fmt_number,
    decimals = 2, suffixing = TRUE) %>% # pattern = "{x}B"
  # tab_footnote(
...
__NOTE:__ This analysis is built from a time series forecast and is not certified by accounting. It is only meant to help visualize trends.
...
* __Model 1 (High):__ Covid data included, and covid period counted as 3/1/2020 - 5/1/2021    
    * This model produces the smallest forecasted traffic, due to the impact of the negative trend.
* __Model 2 (Low):__ Covid data included, leveled negative growth trend  
    * This model producess a forecasted traffic that is close to 2021 traffic.
* __Model 3 (Mid):__ Covid data not included (then forecast 3 years out)  
...
### Data Generation {-} 
Sales data is reported monthly by some of the retailers at the centers. 

### Data Pull from DB {-}
For forecasting purposes we have pulled data for all centers from 2016 until now (2022.01.26).

__Source:__ OpsStats (server: WAREHOUSESQL)  

__Tables:__ [Warehouse].[dbo].[FactAdjustedSales]

* There are some retailers who are only required to report once a year (TJ Max?)  
* The results are not controlled/adjusted by the count of retailers reporting  
* Only currently active centers are included in what is analyzed below.
...
Hist <- CleanSales %>% 
  ...
  group_by(month, All) %>% 
  summarise(traff = sum(sales,na.rm=T)) %>% 
  ungroup()
...
ggplot(data = Hist, aes(month, traff))+
  ...
  labs(title ="Monthly Retailer Revenue Trends Over Time",
       subtitle = "Sales reported monthly",
       y = NULL, x=NULL)
